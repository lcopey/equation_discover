...
class RNNSampler(Sampler):
    def __init__(
        ...
        self(input_tensor, hidden_tensor)

    def call(self, input_tensor: tf.Tensor, hidden_tensor: tf.Tensor):
        output = input_tensor
        state = hidden_tensor
    ...
    def sample(self, n: int):
        ...
        entropies = tf.zeros(shape, dtype=TF_FLOAT_DTYPE)
        log_probs = tf.zeros(shape, dtype=TF_FLOAT_DTYPE)
        ...
        lengths = tf.zeros(n, dtype=TF_INT_DTYPE)
        ...
        while tf.reduce_any(tf.reduce_all(sequence_mask, axis=1)):
            ...
            counters += (
                tf.cast(
                    tf_isin(tokens, self.tokens.two_arity_tensor), dtype=TF_INT_DTYPE
                )
                * 2
            )
            counters += tf.cast(
        ...
        lengths = tf.reduce_sum(tf.cast(sequence_mask, dtype=TF_INT_DTYPE), axis=1)
        sequence_mask_float = tf.cast(sequence_mask[:, :-1], dtype=TF_FLOAT_DTYPE)

        entropies = tf.reduce_sum(entropies * sequence_mask_float, axis=1)
        log_probs = tf.reduce_sum(log_probs * sequence_mask_float, axis=1)
        return sequences, lengths, entropies, log_probs

    def _sample_tokens(self, input_tensor, hidden_tensor, counters, lengths, sequences):
    ...
    def get_parent_sibling(self, sequences, lengths):
        parent_sibling = tf.ones((lengths.shape[0], 2), dtype=TF_INT_DTYPE) * -1
        ...
        for i in range(recent, -1, -1):
            ...
            arity += (
                tf.cast(
                    tf_isin(token_i, self.tokens.two_arity_tensor), dtype=TF_FLOAT_DTYPE
                )
                * 2
            )
            arity += tf.cast(
                tf_isin(token_i, self.tokens.one_arity_tensor), dtype=TF_FLOAT_DTYPE
            )
        ...
        recent_parent_sibling = tf.concat(
            [
                ...
                -1 * tf.ones((lengths.shape[0], 1), dtype=TF_INT_DTYPE),
            ],
            axis=1,
        )
